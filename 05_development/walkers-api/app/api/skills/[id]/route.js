...
export async function GET(request, { params }) {
  ...
  const db = sql()
  const rows = await db`SELECT * FROM skills WHERE id = ${id}`
...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const existing = await db`SELECT * FROM skills WHERE id = ${id}`
    ...
    const rows = await db`
      UPDATE skills SET
        ...
        icon = ${m.icon}, tags = ${m.tags || []},
        config = ${JSON.stringify(m.config || {})},
        content = ${m.content || null}, updated_at = NOW()
...
export async function DELETE(request, { params }) {
  ...
  try {
    const { id } = await params
    const db = sql()
    const rows = await db`DELETE FROM skills WHERE id = ${id} RETURNING id`
    if (!rows.length) return Response.json({ error: 'Not found' }, { status: 404 })

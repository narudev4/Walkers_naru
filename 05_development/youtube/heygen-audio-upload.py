...
使い方:
  browser-use python --file 05_development/youtube/heygen-audio-upload.py
...
# === 設定 ===
# プロジェクトルート: このファイルは 05_development/youtube/heygen-audio-upload.py にあるので parents[2]
# Mac/Windows両対応（ハードコードパスを使わない）
PROJECT_ROOT = Path(__file__).resolve().parents[2]
BASE_DIR = str(PROJECT_ROOT / "output" / "youtube")
BU = os.environ.get("BROWSER_USE_BIN", "/Users/naru/.browser-use-env/bin/browser-use")
...
def main():
    if not SLUG:
        ...
        print("例: HEYGEN_SLUG=what-is-make browser-use python --file 05_development/youtube/heygen-audio-upload.py")
        sys.exit(1)

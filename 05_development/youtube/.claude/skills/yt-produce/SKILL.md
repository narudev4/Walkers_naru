...
📊 スライド: projects/{slug}/slides.pptx
   （自動で開きます）
🔊 音声: projects/{slug}/audio/full.wav
   （自動でFinderで開きます）
...
👤 あなたの作業（2つ）:
  ...
  【スライド確認】
  1. PPTXを開いてスライドを確認してください
  2. 修正したい箇所があれば指示してください
     （内容修正→slides.json編集→エンジン再実行）
  3. OKなら「スライドOK」と言ってください
...
> - ❌ NG: 「不要そうな sleep を消そう」「scroll を高速化しよう」など意図不明の変更（最適化版で実装済）
> - ✅ OK: `HEYGEN_START=N` で再開 → それでも同じ場所で詰まる場合のみ、**ユーザーに相談してから**コードに触る
> - ✅ OK: ユーザーから明示的にリクエストされた改善（実機検証必須・learnings.md に追記）
>
> **理由**: setup.py は既に多数の罠を踏み抜けた実績コード。エラーの多くは UI 状態の一時的ズレで、
> 再開すれば解消する。憶測での修正は過去に踏んだ罠を再発させるリスクが高い（実際に過去発生）。

...
**STEP 6（HeyGen 動画生成）は半自動**。PPTX アップロード・**アバター選択**（本番用：山口鳳汰(背景リアル&スーツ見えるver)）・Create Video・音声アップロード・アバター位置調整は `_shared/heygen-setup.py` ですべて自動化済み。yt-produce は heygen-setup.py の起動コマンドと BGM 設定だけを案内したら終了する。最終的なプレビュー確認 → 生成ボタン押下 → HeyGen 側の 2〜4 時間レンダリング待機は、ユーザーが自分のタイミングで実行する（yt-produce は待機しない）。
...
**手動作業: BGM 追加・最後の生成ボタン押下**
**自動化済み: PPTX アップロード + アバター選択（本番用：山口鳳汰）+ Create Video + 音声アップロード + アバター位置調整（200x200 / 右上角）すべて `heygen-setup.py` で完結**
...
これからHeyGenで動画を生成します。
PPTX→アバター選択→音声アップ→位置調整までは heygen-setup.py が一括で実行します。
CDP Chrome は未起動なら自動で立ち上がるので、ユーザーは何もしなくてOK。
ユーザーが手で触るのは BGM 追加と最後の生成ボタンだけです。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
ここから先（プレビュー確認 → 生成ボタン → 2〜4時間待機）はユーザーが自分のタイミングで実行してください。
yt-produce はここで案内終了。
...
- STEP 1〜5.5 は完全自動進行
- STEP 6 は半自動（heygen-setup.py 起動を案内 → ユーザーがアバター/BGM/プレビュー/生成ボタンを操作）

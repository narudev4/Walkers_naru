---
description: /yt-heygen — HeyGen動画生成スキル
---
...
- スラッグ → `projects/{slug}/audio/scenes/scene01_*.wav` 〜
- 引数なし → エラー（スラッグ必須）
...
HEYGEN_SLUG=what-is-make HEYGEN_PPTX_PATH=projects/what-is-make/slides.pptx /Users/naru/.pyenv/versions/3.13.0/bin/python3 _shared/heygen-setup.py

# Phase 1+2: 全シーン一括処理（音声アップ＋アバター配置）
HEYGEN_SLUG=what-is-make /Users/naru/.pyenv/versions/3.13.0/bin/python3 _shared/heygen-setup.py

# 途中から再開
HEYGEN_SLUG=what-is-make HEYGEN_START=15 /Users/naru/.pyenv/versions/3.13.0/bin/python3 _shared/heygen-setup.py

# ドライラン（操作せず確認のみ）
HEYGEN_SLUG=what-is-make HEYGEN_DRY=1 /Users/naru/.pyenv/versions/3.13.0/bin/python3 _shared/heygen-setup.py
```

## 実装スクリプト: `_shared/heygen-setup.py`
...
`projects/{slug}/heygen-setup-progress.json` で中断→再開。

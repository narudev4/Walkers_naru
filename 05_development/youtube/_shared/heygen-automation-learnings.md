...
## 17. シーン音声は 1.0秒以上必須 — HeyGen アップロードは「> 1 秒」制約（2026-05-20）

### 症状
`claudecode-competitive-advantage` プロジェクトで scene21 を HeyGen にアップロードしようとした際、UI 上で「> 1 秒」のエラーが出てアップロードできなかった。

### 原因
- `script.md` 【スライド21】セクションタイトル 05 のナレーション原稿が `まとめ` の **3文字のみ**
- ElevenLabs が生成した scene21.wav が **約 0.8秒** → HeyGen 制約「> 1 秒」未満で reject

### 一時対処（個別プロジェクト）
ナレーション原稿を膨らませて scene21 を再生成:
```
### 【スライド21】セクションタイトル 05
それでは最後に、本動画のまとめをお話しします。   ← 22文字 → 約3秒
```
シーン22 冒頭の「それではですね、最後にまとめをお話しします。」を削除して被りを回避。

### 恒久対策（再発防止・3層）

**事前防止（核心策・原稿生成時の介入）**:
- `.claude/skills/yt-script/SKILL.md` 「scene の最低秒数」セクションを追加
- セクションタイトル scene は **最低15文字以上 / 約1.5秒以上**
- 単語スライド (`まとめ`, `事例` 等) は必ず文章化

**事後保険（生成パイプラインの assert）**:
- `_shared/yt-voice.py` の Mode 2 (`run_regen`) に `dur < 1.0` で `sys.exit(1)` 追加
- `_shared/yt-split-audio.py` の cut ループに `actual < 1.0` で `short_scenes` 収集 → ループ後に一括エラー停止

**設計判断（参考）**:
- 警告ではなくエラー停止: 1秒未満は HeyGen で必ず失敗するので「黙って続けて後で発覚」より「即停止 → 修正」が圧倒的に早い
- CTA scenes は固定テンプレ (`cta_audio_pcm.wav`) 由来で全て1秒以上を実測確認済み (最小 scene28 = 3.92秒) → 誤検知なし

### 罠ポイント
- **`yt-voice.py` の生成ログには `dur` を表示していたが、閾値 assert がなかった**ため、0.8秒でも `✅` で通過して HeyGen 段階まで気づけなかった
- セクションタイトルのナレーションは「短ければ短いほど自然」と直感的には思えるが、TTS + HeyGen のチェーンでは 1秒未満が NG という非自明な制約がある

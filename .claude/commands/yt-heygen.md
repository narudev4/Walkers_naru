...
| HEYGEN_VERIFY | "1"でスイープ検証モード（アップロードせず全シーンの音声を検証） | "0" |
| HEYGEN_VERIFY_VISUAL | "1"でclaude -p視覚検証を有効化（HEYGEN_VERIFY=1と併用） | "0" |
...
各シーンのアップロード直後に**DOM検証**を実行:
1. 左パネルに表示された.wavファイル名を読み取り
2. 期待ファイル名と照合（完全一致 / プレフィックス一致 / ミスマッチ / 不在）
3. ミスマッチ時は**自己修復**（音声削除→再アップロード→再検証、最大2回リトライ）
4. DOM検証が曖昧な場合は`claude -p`でスクリーンショットを視覚解析（`HEYGEN_VERIFY_VISUAL=1`時）
...
```bash
# DOM検証のみ（高速、2-3分）
HEYGEN_SLUG=xxx HEYGEN_VERIFY=1 python3 output/youtube/_shared/heygen-setup.py

# DOM + claude -p 視覚検証（8-10分）
HEYGEN_SLUG=xxx HEYGEN_VERIFY=1 HEYGEN_VERIFY_VISUAL=1 python3 output/youtube/_shared/heygen-setup.py
```
...
スクリプトは起動時にロックファイルを作成し、別プロセスの同時実行を防止。
...
- 対策:
  - 各シーンのアップロード後に伝播チェック（実際のファイル名で判別）
  - 伝播と次シーンの正しい音声を区別（ファイル名比較）
  - 伝播した音声は自動削除
  - アップロード後のDOM検証で正しいファイルが入ったか確認
  - ミスマッチ時は自己修復（削除→再アップ→再検証）
...
| 音声アップ後に後続シーンに伝播 | 伝播チェック + ファイル名判別 + 自動削除 |
| 音声ミスマッチ（別ファイルが入る） | DOM検証 + 自己修復（削除→再アップ→再検証） |
| 二重起動でレースコンディション | PIDロック + ad-hocスクリプト禁止 |

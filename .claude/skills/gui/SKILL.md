...
| ステップ | ツール | 用途 |
|---------|--------|------|
| Phase 1 | Read (built-in) | `output/gui/state/` のstate JSONファイル読み込み |
| Phase 1 | Edit (built-in) | ソースファイルへの変更反映 |
| Phase 3 | Read (built-in) | ローカルファイル読み込み |
| Phase 3 | `gmail_search_messages` / `gmail_read_message` (Gmail MCP) | メールデータ取得 |
| Phase 3 | `gcal_list_events` / `gcal_get_event` (Google Calendar MCP) | カレンダーデータ取得 |
| Phase 3 | `google_sheets_read` / `google_sheets_get_info` (自作Google MCP) | スプレッドシートデータ取得 |
| Phase 3 | `google_docs_get` (自作Google MCP) | ドキュメント取得 |
| Phase 3 | `google_drive_search` (自作Google MCP) | Drive内ファイル検索（ID不明時） |
| Phase 3 | `API-post-search` / `API-query-data-source` (Notion MCP) | Notionデータ検索 |
| Phase 3 | `misoca_list_invoices` / `misoca_list_estimates` (Misoca MCP) | 請求書・見積書データ取得 |
| Phase 4 | Write (built-in) | HTMLファイル生成・保存 |
| Phase 5 | Bash (`open`) | ブラウザで自動表示 |
| Phase 6 | Read (built-in) | state JSON読み込み |
| Phase 6 | Edit (built-in) | ソースファイル更新 |
...
2. ブラウザで開く（環境に応じて `open` / `start` / URLを案内）
3. ユーザーに以下を伝える:
   - 「ブラウザで開きました」（または「以下のURLをブラウザで開いてください」）
   - URL: `file:///<プロジェクトルートの絶対パス>/output/gui/{name}.html`

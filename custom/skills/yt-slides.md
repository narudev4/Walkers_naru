...
## テキスト参照ルール（スライド表示 / ナレーション分離・CRITICAL）

台本は各スライドに「**スライド表示:**」と「**ナレーション:**」の2種類のテキストを持つ。

- **スライド面に表示するテキスト** → 「**スライド表示:**」セクションから取得する。原語・正式表記（API, Python, Make, ノーコード等）で書かれている。
- **スピーカーノート** → 「**ナレーション:**」セクションから取得する。読み仮名・TTS向け表記（エーピーアイ, パイソン, メイク, のーこーど等）で書かれている。

### なぜ分離するか

台本はElevenLabs TTS向けに読み仮名化されている（例: `API → エーピーアイ`）。スライドにそのまま表示するとダサいため、スライド面には原語表記を使う。

### 「スライド表示:」が無いスライドの扱い

- 固定スライド（タイトル、自己紹介、チャンネル登録、CTA）は見本PPTXからXMLコピーするため、台本のテキストを参照しない
- セクションタイトルスライドは `### 【スライドN】セクションタイトル: ○○` の見出しからタイトルを取得する
- 旧フォーマットの台本（「スライド表示:」が無い）では、ナレーションテキストから内容を推測してスライドを生成する（従来動作）

### スライド生成時の注意

- **スライド面にカタカナ読み仮名（のーこーど、エーピーアイ等）を絶対に表示しない**
- 「スライド表示:」に書かれた要点・箇条書きをそのまま使ってカード・テーブル・テキストボックスを生成する

## 処理フロー

1. 台本MDを読み込む
2. 各スライドの「**スライド表示:**」と「**ナレーション:**」を分離して把握する
3. 既存テンプレート `output/slides/generate_heygen_slides.py` を Read で読み込む
4. 台本の内容に合わせてPython生成スクリプトを作成（**スライド面のテキストは「スライド表示:」から取得**）
5. **bash heredoc方式で実行**（`python3 << 'PYEOF'` ... `PYEOF`）※ファイル書き込みだとエンコーディング問題が起きるため
6. `open` コマンドでPPTXを開く
...
- PPTX: `output/youtube/{slug}/{slug}.pptx`
...
- [ ] **スライド面にカタカナ読み仮名（のーこーど、エーピーアイ、メイク等のTTS表記）が表示されていないか**
- [ ] **「スライド表示:」セクションのテキストがスライド面に正しく反映されているか**
...
from pptx import Presentation
prs = Presentation("output/youtube/{slug}/{slug}.pptx")

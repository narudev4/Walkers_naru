...
HEYGEN_SLUG=what-is-make HEYGEN_PPTX_PATH=output/youtube/what-is-make/slides.pptx /Users/naru/.pyenv/versions/3.13.0/bin/python3 05_development/youtube/heygen-setup.py
...
- **ナビゲーションにショートカットを入れない**
  - 「Uploadボタンが見えてるなら目的のシーンがアクティブ」みたいな推測ロジックは禁止
  - 必ず左パネルの番号検索→スクロール→クリックを毎回愚直に実行する
  - 実例: シーン15処理中にシーン16のUploadボタンでマッチし、15の音声が16にアップされる事故があった

- **左パネルのスクロール範囲はscrollHeightに合わせる**
  - 36シーンで scrollHeight≈7299px。スクロール回数を固定にせず、`scrollTop > scrollHeight * 0.95` で停止する方式で十分な回数を確保する
  - 検索はスクロール前に実行される点に注意（最後のスクロール位置は検索されない）

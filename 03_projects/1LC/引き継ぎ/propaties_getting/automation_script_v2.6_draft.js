/*****************************************************************
 * エリア別物件取得 — v2.6(ドラフト): 部分書き込み対応
 *
 * 背景（2026-07-11 v2.5実機テストで判明）:
 *   v2.5は「1エリアでもfetch例外(socket hang up)が起きたら、
 *   以降の未処理エリアも、既に成功していたエリアの書き込みまで
 *   全部スキップする」設計だった（v2.4から引き継いだ「ランタイム
 *   汚染対策」）。実機で2回テストしたところ、2回とも別々の
 *   エリア（1回目:東京23区、2回目:千葉県）でfetch例外が発生し、
 *   2回とも新規作成0件に終わった。
 *
 *   「外部fetch失敗でAirtable自身のAPI呼び出しまで壊れる」という
 *   前提はv2.3時代の経験則由来とみられるが、当時の一次情報が無く、
 *   実際に書き込みを試して壊れるかどうかは今回も未検証のまま
 *   全部スキップされていた。1回の失敗で37エリア分の成果が毎回
 *   消えるのは明らかに非合理なため、本バージョンで「疑わしきは
 *   罰せず」の設計に変更する。
 *
 * v2.5 からの変更点:
 *   1. 「ランタイム汚染」フラグによる全体停止・書き込みスキップを廃止
 *   2. フェーズ1: 全エリアを1回ずつ試行。fetch失敗したエリアは
 *      「今回は取得できなかった」と記録し、他のエリアの処理には
 *      影響させず続行する
 *   3. フェーズ2: フェーズ1で失敗したエリアのみ、残りのfetch予算
 *      （上限50回/実行）内で再試行する
 *   4. 取得できたデータは、一部エリアが失敗していても必ず書き込む
 *   5. 再試行しても失敗したエリアは、次回の定期実行（6時間後）に
 *      委ねる。1回の実行で100%揃えることを狙わない設計に変更
 *
 * 未解決の論点（今回は対応スコープ外・新サーバー移行時に根本対応）:
 *   Airtable公式コミュニティは「1スクリプトで複数fetchをループする
 *   とタイムアウトしやすいので分割すべき」と案内しており、今の設計
 *   （1スクリプトで38回fetch）自体がAirtable非推奨のパターン。
 *   ただし1LCは新サーバー移行が近い見込み（2026-07-15火曜定例で
 *   費用感を協議予定）のため、Airtable Automation自体を分割する
 *   ような延命的な設計変更は行わず、このまま新サーバー移行を待つ
 *   方針とした（2026-07-11 naruと合意）。
 *****************************************************************/

const BATCH     = 50;
const API_URL   = "https://www.1lcinc.com/syuuekibukken/php/json_output.php";
const HKENSU    = 500;   // 東京都実測248件・他エリア実測最大80件に対する安全マージン

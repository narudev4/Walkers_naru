/*****************************************************************
 * エリア別物件取得 — v2.8(ドラフト): hkensu引き上げ + 土地権利フィルタ
 *
 * 背景（2026-07-13 岩下様との往復で確定した仕様）:
 *
 * 1. hkensu=500 は余裕が少なすぎるとご指摘（459/500=約92%消化）。
 *    「掲載物件数が少し増えるだけで再び静かに取りこぼす」懸念のため
 *    1,000に引き上げ（チタン様推奨レンジ内。他の主要エリアは実測
 *    最大80件程度で、東京都のみ突出するため1,000でも十分な余裕）。
 *
 * 2. 土地権利フィルタの仕様:
 *    - 岩下様の運用方針: マッチングでは土地権利が「所有権」の物件
 *      のみを対象とする
 *    - ただし「値が空欄・未取得の物件は除外せず所有権として取り込む」
 *      （空欄まで除外すると、全物件に値が行き渡るまで取込がほぼ
 *      全滅するため）
 *    - 既存Airtableレコード（借地物件含む）は、遡及登録により
 *      「土地権利」の値は更新するが、レコード自体は削除しない
 *      （岩下様側のマッチングシステムで除外処理を行うため、
 *      Walkers側での削除は不要）
 *
 *    → 上記2点を両立するため、フィルタは「新規作成の可否判定」
 *      にのみ適用し、「既存レコードの更新」には適用しない設計とする。
 *      既存レコードは常に無条件で更新（土地権利の値も含めて）、
 *      新規作成のみ「土地権利が空欄 or 所有権」の場合に限定する。
 *
 * v2.7 からの変更点:
 *   1. HKENSU: 500 → 1,000
 *   2. creates/updates 振り分けで、新規作成時のみ
 *      「土地権利が所有権以外なら作成しない」フィルタを追加
 *      （既存レコードの更新には適用しない）
 *
 * 未対応:
 *   - Airtable側の「土地権利」選択肢に「所有権」「旧法賃借」
 *     「旧法地上」の3つを事前登録する必要あり（naru対応、未実施）。
 *     岩下様のメールでは「普通借地」「定期借地」という値の可能性にも
 *     言及があったが、実測（573件）では未確認のため選択肢への
 *     事前追加は見送り、未登録値エラーが出たら都度追加する運用とする
 *****************************************************************/

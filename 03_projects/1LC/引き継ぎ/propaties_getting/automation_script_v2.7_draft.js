/*****************************************************************
 * エリア別物件取得 — v2.7(ドラフト): 「土地権利」フィールド追加
 *
 * 背景（2026-07-13 岩下様より依頼）:
 *   チタン様がAPIに「土地権利」（所有権/旧法賃借/旧法地上）を
 *   追加対応済み（2026-07-10完了）。岩下様の運用方針は「土地権利が
 *   所有権の物件のみをマッチングで取り込む」で、判定に必要な
 *   「土地権利」のみAirtableへの追加を依頼された（「借地期間残存」
 *   「地代」は不要とのこと）。
 *
 *   実測値（価格5,000万円以上・573件中）: 土地権利の値は
 *   「所有権」「旧法賃借」「旧法地上」の3種のみ、487件に値あり
 *   （86件は空値）。既存の「建物構造」「種別」と同様、Airtable側は
 *   単一選択（Single select）フィールドとして事前に3選択肢を
 *   登録する必要がある（未登録値だと新規作成エラーになる、
 *   ALCの前例と同じ）。
 *
 * v2.6 からの変更点:
 *   1. 「土地権利」フィールドの書き込みを追加（Single select形式
 *      {name: 値} で送信。値が無い場合は書き込まない）
 *
 * 未対応（岩下様の回答待ち）:
 *   「所有権以外の物件はAirtable取込段階でそもそも対象外にする」
 *   という理解が正しいか確認中。正しい場合、本スクリプトに
 *   「土地権利が所有権以外なら ingest しない」というフィルタを
 *   追加する必要があるが、本バージョンでは未実装（返信待ち）。
 *****************************************************************/

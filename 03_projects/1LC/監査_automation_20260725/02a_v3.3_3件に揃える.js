// ========================================
// 物件マッチング v3.3 — マッチング件数をメール表示件数（3件）に揃える
// Airtable Automation Script (Step 1)
//
// v3.2 → v3.3 の変更（2026-07-31）:
//   マッチングは5件取得していたが、メールテンプレート（lc-v3-prod / ID 45535824）は
//   hero 1件 + subs 2件 = **3件しか表示していない**。
//   にもかかわらず取得した5件すべてが Webhook 経由で ★物件紹介履歴 に
//   「紹介済み」として記録されるため、**表示されなかった2件は二度と紹介されない**。
//   2026-07-25 の実測で 85人中78人がちょうど5件記録 ＝ 1日あたり約156件の物件が
//   誰にも見られないまま焼かれていた。
//
//   変更点は break 条件の 5 → 3 のみ（1箇所）。
//
//   副次的な効果:
//     - 走査が早く打ち切られるため1人あたりの処理時間が短くなる
//     - 画像の HEAD チェックが最大5回→3回になり fetch 消費が減る
//     - 「3件そろわない投資家」は5件基準より少ないため scanTruncated も減る
//
//   注意: 候補が3件しかない状態で画像ありを hero に選ぶため、
//   hero に画像が付く確率は5件から選ぶ場合より下がる。
//
// 【元に戻す方法】この行を 3 → 5 に戻す（02a_v3_修正版.js が v3.2 の全文）
// ========================================
// 以下は v3.2 のヘッダ（経緯の記録）
// ========================================
...
for (let record of sortedRecords) {
  // 【v3.3】メールは hero 1件 + subs 2件 = 3件しか表示しないため 3 で打ち切る。
  // 5件取得していた頃は、表示されない2件も履歴に「紹介済み」として記録され
  // 二度と紹介できなくなっていた（1日あたり約156件）。

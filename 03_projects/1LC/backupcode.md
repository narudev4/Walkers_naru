// テーブルの参照
let tableBukken = base.getTable("物件エリア");
let tableTochi = base.getTable("土地大臣コード");

// フィールド名
let bukkenAreaField = "エリア名"; // "物件エリア"のエリア名フィールド
let tochiPrefectureField = "都道府県"; // "土地大臣コード"の都道府県フィールド
let tochiAddressCodeField = "住所コード"; // "土地大臣コード"の住所コードフィールド
let bukkenRefField = "土地大臣コード（ref）"; // "物件エリア"のリンクフィールド

// 土地大臣コードテーブルのすべてのレコードを取得
let tochiRecords = await tableTochi.selectRecordsAsync();
let tochiMap = new Map();

// 都道府県をキーにして土地大臣コードのレコードIDリストをマッピング
for (let record of tochiRecords.records) {
    let prefecture = record.getCellValue(tochiPrefectureField);
    if (prefecture) {
        if (!tochiMap.has(prefecture)) {
            tochiMap.set(prefecture, []);
        }
        tochiMap.get(prefecture).push(record.id); // 同じ都道府県のレコードをリストに追加
    }
}

// 物件エリアテーブルのすべてのレコードを取得
let bukkenRecords = await tableBukken.selectRecordsAsync();
let updates = [];

for (let record of bukkenRecords.records) {
    let bukkenArea = record.getCellValue(bukkenAreaField);
    if (bukkenArea && tochiMap.has(bukkenArea)) {
        let linkedRecords = tochiMap.get(bukkenArea).map(id => ({ id })); // レコードIDリストをリンク形式に変換
        updates.push({
            id: record.id,
            fields: {
                [bukkenRefField]: linkedRecords // 複数レコードをリンク
            }
        });
    }
}
...
while (updates.length > 0) {
    await tableBukken.updateRecordsAsync(updates.slice(0, 50));
    updates = updates.slice(50);
}

output.text("物件エリアに住所コードが正常に紐付けられました！");

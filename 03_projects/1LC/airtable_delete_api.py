#!/usr/bin/env python3
# 1LC Airtable 承認セット 一括削除（REST API版）① ★物件紹介履歴
# - DRY RUN（既定）: 件数を数えるだけ・1件も消さない
# - 対話削除: python3 airtable_delete_api.py --delete         → "yes" 入力で実行
# - 無人削除: python3 airtable_delete_api.py --delete --yes   → 確認スキップ（寝てる間OK）
# - トークンは環境変数 AIRTABLE_PAT か credentials/airtable-1lc-pat.txt から読む
import os, sys, time, json, urllib.request, urllib.parse, urllib.error

# ===== 設定 =====
BASE_ID  = "appksEWIuKl7N2ftS"
TABLE_ID = "tblMieyctX1o6LI7X"   # ★物件紹介履歴
VIEW_ID  = "viwMmHjysYqs0ZkAb"   # 承認ビュー（紹介日時<2026/3/1）
# ================
...
def main():
    print("対象: TABLE=%s VIEW=%s (★物件紹介履歴)  %s" % (TABLE_ID, VIEW_ID, time.strftime("%Y-%m-%d %H:%M:%S")))
    ids = collect_ids()

...
WORK = Path('/Users/naru/Walkers_naru/03_projects/ 株式会社ＡＤｅｅｄ/minutes/2026-05-29_初回MTG')
PARTS = WORK / 'parts'
SEG = 600          # 分割秒（Phase 3 と一致）
N_PARTS = 4        # 36分録画 → 10分×4
...
clean_segs, removed = clean_consecutive(segs)
...
report = []
report.append(f"有効={len(clean_segs)}seg / 文字数={sum(len(s['text']) for s in clean_segs)} / レンジ {hms(clean_segs[0]['start'])}〜{hms(clean_segs[-1]['end'])}")
report.append(f"連続反復除去: {len(removed)}件")
for st, n, t in removed:
    report.append(f"  {hms(st)} x{n} {t}")
report.append(f"30秒超ギャップ {len(gaps)}件:")
for st, en, g in gaps:
    report.append(f"  {hms(st)}〜{hms(en)} ({g:.0f}s)")
(WORK/'_merge_report.txt').write_text('\n'.join(report))
print('\n'.join(report))

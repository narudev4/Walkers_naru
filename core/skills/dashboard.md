...
1. まず `data.json` を最新化する:
   ```bash
   cd <PROJECT_ROOT>/05_development/walkers-dashboard && bash refresh.sh
   ```
2. サーバーをバックグラウンドで起動する:
   ```bash
   cd <PROJECT_ROOT>/05_development/walkers-dashboard && python server.py > <PROJECT_ROOT>/output/walkers-dash.log 2>&1 &
   ```

...
| ステップ | ツール | 用途 |
|---------|--------|------|
| Step 1 | `google_drive_search` (自作Google MCP) | Google Docs上の提案書検索 |
| Step 1 | `google_docs_get` (自作Google MCP) | 提案書本文取得 |
| Step 1 | Glob + Read (built-in) | ローカル提案書・facts.md参照 |
| Step 3 | Read (built-in) | `slide_base.py` の確認 |
| Step 3 | Write (built-in) | 生成スクリプト保存 |
| Step 3 | Bash (`pip install python-pptx`) | 依存ライブラリインストール（初回のみ） |
| Step 3 | Bash (`python generate_*_slides.py`) | スクリプト実行・PPTX生成 |
| Step 4 | Bash (`open`) | 生成PPTXを開く |
...
# === 保存 ===
output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "{ファイル名}.pptx")
...
open "output/slides/{クライアント名}_{案件名}_提案スライド.pptx"
```

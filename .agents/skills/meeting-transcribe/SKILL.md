---
description: MTG録画/音声から逐語＋構造化議事録を生成し、案件minutes/に保存→S3同期。tl;dv代替。
---
...
ローカル録画 or Drive の会議録画から、**mlx-whisper で逐語**＋**Codex で構造化議事録**を生成し、`03_projects/{案件}/minutes/{日付}_{会議名}/` に保存。`/sync-up` 連鎖で `walkers-context-prod` に反映され、`context-view` 経由でスマホ Safari から案件ごとに閲覧可能。

Gemini メモの「内容が省略される」問題を **逐語全文＋構造化要約の二段構え** で解決し、tl;dv の有料サブスクを置き換える狙い。
...
**❌ `--initial-prompt`（固有名詞ヒント）は使わない（2026-05-28 実証で逆効果）。** プロンプト語（例「福岡県」）に引っ張られ、無音区間でその語を連発する暴走を誘発する。固有名詞の揺れは Phase 7 で Codex が文脈補正＋「固有名詞・要確認」テーブルに記録する運用でカバー。
...
3. **30秒超ギャップ検出**（コンテンツ欠損 vs 画面共有デモ中の正当な無音を Codex が判断するQAゲート）
...
### Phase 9: wav削除＋S3同期（連鎖規約）

**まず大きな wav・中間ファイルを削除**（pre-sync-guard が 100MB 超を弾く。重複JSONも整理）:
```bash
...
スキル末尾で **必ず** `/sync-up` を呼ぶ（AGENTS.md の連鎖規約）。

```
/sync-up
```

**機密の注意**: 先方退出後の内部戦略トーク（入札の立ち回り等）を minutes.md に含めると、S3同期で**スマホ閲覧用HTMLになり、view bucket をクライアントや中間者と共有していると漏れる**。機微な内部戦略は、同期する議事録に残すかローカル限定メモに分けるかを**ユーザーに確認**する。

### Phase 10: 完成報告

`~/bin/mo "$WORK/minutes.md" "$WORK/transcript.txt"` でローカルプレビュー URL を出して報告。**要確認事項（話者特定・日程・別件混入・機密）は必ず明示**してユーザーの補正を促す。精度が問題になった場合は before/after の文字数差で改善を示す。

## 話者分離の運用方針

**v1（現行）**: Phase 1 の参加者ヒアリング ＋ Phase 7 で Codex が文脈推測（`?` 付き）。多人数で確証が無ければ組織単位（当方/先方）に退避。
...
- 同案件の過去 minutes から固有名詞辞書を抽出（`--initial-prompt` ではなく Phase 7 の Codex 補正の参照辞書として使う）

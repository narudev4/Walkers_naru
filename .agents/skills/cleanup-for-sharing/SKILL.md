...
|---------|--------|------|
| ② | Bash (`rsync`) | ディレクトリ複製 |
| ③-A | Bash (`rm`, `find`) | ファイル削除一括実行 |
| ③-B | Read (built-in) | 各ファイルの現在内容確認 |
| ③-B | Edit / Write (built-in) | ファイル内容の書き換え |
| ④ | Grep (built-in) | 個人情報残留チェック |
| ⑤ | Write (built-in) | SETUP.md生成 |
| ⑦ | Bash (`zip`) | zip化 |
| ⑧ | Bash (`rm`) | 一時ファイル削除 |
...
```bash
rsync -a --exclude='.git' ~/Walkers_full/ ~/Desktop/Walkers_template_YYYYMMDD/
```
...
```bash
TDIR="$HOME/Desktop/Walkers_template_YYYYMMDD"
...
| `.Codex/settings.local.json` | `permissions.allow` → 空配列 `[]` |
...
| `AGENTS.md` | 個人パス→`<プロジェクトルートの絶対パス>`、マネージドコネクタUUID削除、接続状態を「要設定」に統一 |
| `.Codex/commands/gui.md` | 個人絶対パスが残っていれば `file:///<プロジェクトルートの絶対パス>/...` に修正 |
| `.Codex/commands/dashboard.md` | 絶対パス → `<プロジェクトルート>` |
| `.Codex/commands/schedule-adjust.md` | 個人メールアドレス → `<個人Googleアカウント>` |
...
| 対象 | 理由 |
|------|------|
| `AGENTS.md` | 業務マニュアル（組織共通） |
| `.Codex/commands/` (22ファイル) | スキル定義（組織共通） |
| `.Codex/agents/` (2ファイル) | エージェントチーム定義 |
...
```
# ↓ 以下は検出パターンの例。実際のマスターインスタンスのオーナー情報に合わせて修正すること
...
```bash
cd ~/Desktop && zip -r Walkers_template_YYYYMMDD.zip Walkers_template_YYYYMMDD/ -x "*.DS_Store"
```

- zip完了後、**デスクトップ上のファイルパスとサイズ**を報告する
...
```bash
rm ~/Desktop/Walkers_template_YYYYMMDD.zip
rm -rf ~/Desktop/Walkers_template_YYYYMMDD/
```

...
| Step 3 | Write | 設定ファイル・AGENTS.md・メモリの生成 |
...
```
① 対象プロジェクト調査 → ② ヒアリング + 構成確認（承認必須） → ③ scaffold + ファイルコピー → ④ Codex環境整備 → ⑤ Git初期化 → ⑥ 完了報告
```
...
4. **メモリファイル** — `~/.Codex/projects/` 配下の当該プロジェクトのメモリディレクトリ内の関連メモリ
...
# メモリ内の関連情報
grep -rl "{keyword}" ~/.Codex/projects/*/memory/
```
...
以下の構成で `~/プロジェクト名/AGENTS.md` を作成する:
...
メモリの配置先: カーブアウト先のルートに `.Codex/` ディレクトリを作成し、その中にメモリを配置。
（Codexが自動的に `~/.Codex/projects/{パスベースのプロジェクト名}/memory/` を使うが、
初回起動時にコンテキストがあるよう AGENTS.md 内に主要情報を記載しておく）

### 4-3. スキル（コマンド）作成

プロジェクトで頻繁に使うワークフローがあれば `.Codex/commands/` にスキルを作成する。
...
4. **AGENTS.mdは必ず作る** — Codexが自走できる状態にすることがカーブアウトの目的
